package com.abc.ordersystem.common.configs;
...
//        redis에 value를 String으로 만들어서 저장하겠다.
    ...
    public RedisTemplate<String, String> stockRedisTemplate(@Qualifier("stockInventory") RedisConnectionFactory redisConnectionFactory){
        RedisTemplate<String, String> redisTemplate = new RedisTemplate<>();
        redisTemplate.setKeySerializer(new StringRedisSerializer()); // 이건 String으로 받아오는거고, json, dto로 받아올때는 Jackson-- 로
        redisTemplate.setValueSerializer(new StringRedisSerializer());
        redisTemplate.setConnectionFactory(redisConnectionFactory);
        return redisTemplate;
    }
...
//    호출구조 : RedisMessageListenerContainer -> MessageListenerAdapter -> SseAlarmService(MessageListener)
    ...
    public RedisMessageListenerContainer redisMessageListenerContainer(@Qualifier("ssePubSub") RedisConnectionFactory redisConnectionFactory,
        ...
        container.setConnectionFactory(redisConnectionFactory); // redisConnectionFactory 주입받아오기
        // 알림 종류가 다를 수 있음. -> 채널이 여러개가 됨.
        container.addMessageListener(messageListenerAdapter, new PatternTopic("order-channel"));
//        container.addMessageListener(messageListenerAdapter, new PatternTopic("member-channel"));
//        만약에 여러 채널을 구독해야 하는 경우, 여러 개의 PatternTopic을 add하거나, 별도의 Listener Bean 객체 생성
        return container;
    }
