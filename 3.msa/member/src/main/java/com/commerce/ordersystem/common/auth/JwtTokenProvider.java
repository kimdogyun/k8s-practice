package com.commerce.ordersystem.common.auth;
...
public class JwtTokenProvider { // 토큰 생성
    ...
    @Value("${jwt.secretKey}")
    private String st_secret_key; // 디코딩 + 암호화 필요

    @Value("${jwt.expiration}")
    private int expiration;


    //    인코딩된 문자열 -> 디코딩 된 문자열 -> HS512알고리즘으로 암호화
    ...
    //    생성자 호출 이후에 아래 메서드를 실행하게 함으로써, @Value 시점보다 늦게 실행되게 하여 값주입의 문제 해결.
    @PostConstruct
    public void init(){
...
//                아래 3가지 요소는 페이로드
                .setClaims(claims)
                .setIssuedAt(now)
//                30분*60초*1000밀리초 : 밀리초 형태로 변환
                .setExpiration(new Date(now.getTime() + expiration*60*1000L))
//                secret 키를 통해 서명값(signature) 생성
                .signWith(secret_key)
                .compact();
        return token;
    }
